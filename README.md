...
- SQL 고급
    - 트랜잭션 : All or Nothing
    - 트랜잭션 속성 : A(atomicity), C(consistency), I(Isolation), D(durability)
    - TCL(TRANSACTION CONTROL LANGUAGE) : BEGIN, COMMIT, ROLLBACK, TRAN[SACTION], SAVE
    
    - SQL Server 는 기본적으로 Auto Commin(시스템이 자동으로 트랜잭션을 걸어줌)
    - SSMS > 도구 > 옵션 > 쿼리 실행 > SQL Server > ANSI
        -> SET IMPLICIT_TRANSACTIONS, 체크 후 프로그램 재시작


    - 트랜잭션 : 로직 처리시, 다른 트랜잭션의 간섭을 받지 않기 위한 것(LOCK), 중요한 데이터 수정시 잘못된 변경을 방지(ROLLBACK)

- 데이터베이스 
    - 설계(모델링) 순서 : 개념 설계 -> 논리 설계 -> 물리 설계
    1.  개념 모델링 : 요구 사항을 받으면서 정해지지 않은 여러 개체들을 정립화 하는 단계
    2.  논리 모델링 : 기본키 지정, 외래키 지정, 관계 정립, 속성들 이름(한글) 개체를 정하는 단계
    3.  물리 모델링 : DB에 맞춰서 컬럼 이름, 컬럼 데이터 타입 및 크기 지정, DB에 대한 검토로 테이블을 만들기 직전의 설계를 완성

    - ER 모델링 : ERD를 그리기 위한 기본 이론

- 인덱스 예제
    - PK나 인덱스가 없는 상태에서 성능 문제를 체크하기 위함!!
    - 인덱스가 설정되면 성능이 어떤지 비교해보자!!
...
- 정규화
    ...
    - 이상 현상이 생기는 테이블을 분리해서 해결
    - 기본키와 함수 종속성을 파악
    - 1 정규형 : 도메인이 원자값을 가짐
    - 2 정규형 : 기본키가 아닌 속성이 기본키에 완전 종속일때(학생번호[PK], 강좌 이름 --> 성적을 결정)
    - 3 정규형 : 기본키가 아닌 속성이 기본키에 비 이행적으로 종속할때  (학생번호 -> 강좌이름 -> 수강료 [이행종속성] : 이것이 일어나는 것을 방지해야함!)
    - BCNF 정규형 : 함수 종속성 X -> Y가 성립할때 모든 결정자 X가 후보키(기본키가 될 수 있는 속성) 이면
    - 보통 BCNF 까지 정규화를 한다 --> 왠만하면 다 해결되어 있음!!
    - 5 정규형 (조인 종속성, 무손실 분해)
